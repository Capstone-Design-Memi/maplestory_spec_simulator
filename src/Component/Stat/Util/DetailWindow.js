...
export const DetailWindow = ({ xy, element }) => {
  const text = Object.entries(DetailText).find(
    ([key, value]) => key === element
  );
          ...
          <div style={{ width: "90%", margin: "0 auto" }}>
            ...
            {mainStatArr.includes(element) && (
              <>
                ...
                <SubText>
                  유니온 공격대 : {statInfo.base.union[element]}
                </SubText>
                <SubText>칭호 : {statInfo.base.title[element]}</SubText>
                <br />
                <SubText>[% 수치]</SubText>
                <SubText>
                  장비 아이템 : {statInfo.persent.item[element + "P"]}%
                </SubText>

                <br />
                <SubText>[% 미적용수치]</SubText>
                <SubText>
                  헥사 스탯 : {statInfo.noPersent.hexa[element]}
                </SubText>
                <SubText>
                  유니온 공격대 : {statInfo.noPersent.union[element]}
                </SubText>
                <SubText>
                  하이퍼 스탯 : {statInfo.noPersent.hyper[element]}
                </SubText>
                <SubText>
                  아케인 심볼 : {statInfo.noPersent.arcane[element]}
                </SubText>
                <SubText>
                  어센틱 심볼 : {statInfo.noPersent.authentic[element]}
                </SubText>
              </>
            )}
          </div>
      ...
      <>
        ...
        <Line>
          <div style={{ width: "90%", margin: "0 auto" }}>
            ...
            <br></br>
            <SubText>[적용 중인 수치]</SubText>
            {base.title[element] > 0 && (
              <SubText>칭호 : {base.title[element]}%</SubText>
            )}
            {base.skill[element] > 0 && (
              <SubText>스킬 : {base.skill[element]}%</SubText>
            )}
            {noPersent.hexa[element] > 0 && (
              <SubText>헥사스텟 : {noPersent.hexa[element]}%</SubText>
            )}
            {base.item[element] > 0 && (
              <SubText> 장비아이템 : {base.item[element]}%</SubText>
            )}
            {base.union[element] > 0 && (
              <SubText>
                유니온 공격대 : {base.union[element] + noPersent.union[element]}
                %
              </SubText>
            )}
            {noPersent.ability[element] > 0 && (
              <SubText>어빌리티 : {noPersent.ability[element]}%</SubText>
            )}
            {noPersent.hyper[element] > 0 && (
              <SubText>하이퍼 스탯 : {noPersent.hyper[element]}%</SubText>
            )}
            {/* <SubText>성향 : </SubText> */}
          </div>
        </Line>
          ...
          <div style={{ width: "90%", margin: "0 auto" }}>
            ...
            <SubText>
              ...
              {base.skill[element] +
                ...
                base.item[element] +
                base.union[element] +
                noPersent.union[element] +
                noPersent.hyper[element] +
                base.title[element] +
                noPersent.ability[element]}
            </SubText>
