...
export const GetSumOfStat = () => {
  ...
  for (let i = 0; i < mainStatArr.length; i++) {
    ...
    result[index] = Math.floor(
      (base.base[index] +
        ...
        base.union[index] +
        base.title[index]) *
        (1 + persent.item[index + "P"] / 100) +
        noPersent.hexa[index] +
        noPersent.union[index] +
        noPersent.hyper[index] +
        noPersent.arcane[index] +
        noPersent.authentic[index]
    );
  }
  ...
  result.statAtk = Math.floor(
    ((result[mainStat] * 4 + result[subStat]) / 100) *
      result[mainAtk] *
      ((100 + result.dmg) / 100) *
      ((100 + result.finalDmg) / 100) *
      constanceArr[0] *
      constanceArr[1]
  );
  ...
  if (thiefArr.includes(constanceArr[2])) {
    result.statAtk = Math.floor(
      ((result[mainStat] * 4 + result[subStat] + result.str - 2.2) / 100) *
        result[mainAtk] *
        ((100 + result.dmg) / 100) *
        ((100 + result.finalDmg) / 100) *
        constanceArr[0] *
...
export const GetMainStat = () => {
  ...
  return Object.entries(JobList).find(([key, value]) =>
    value.includes(information.job)
  )[0];
};
...
export const GetStatInformation = () => {
  ...
  result.noPersent.ability = {
    ...result.noPersent.ability,
    ...information.spec.abilities,
  };
