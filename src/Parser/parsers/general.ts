...
const CHARACTER_LEVEL_SELECTOR =
  "div.char_info_top > div.char_info > dl:nth-child(1) > dd";
const CHARACTER_IMAGE_SELECTOR =
  "div.char_info_top > div.char_info > div.char_img > div > img";
...
export class GeneralInformationParser {
  ...
  private parseLevel(node: HTMLElement): number {
    const level = node
      .querySelector(CHARACTER_LEVEL_SELECTOR)
      ?.text.replace(/[^\d]/g, "");
  ...
  private parseTraits(node: HTMLElement): Traits {
    ...
    const [ambition, insight, willpower, diligence, empathy, charm] =
      traitNodes.map((n) => parseInt(n.text));
    return {
      ambition,
      insight,
      willpower,
