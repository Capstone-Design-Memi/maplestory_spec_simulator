...
import {
  ...
  NotValidSpecPageError,
} from "../errors";

const CHARACTER_LINKS_SELECTOR =
  "div.rank_table_wrap > table > tbody > tr > td.left > dl > dt > a";
const EQUIPMENT_LINK_SELECTOR =
  "#container > div.con_wrap > div.lnb_wrap > ul > li:nth-child(3) > a";
const PET_LINK_SELECTOR =
  "#container > div.con_wrap > div.lnb_wrap > ul > li:nth-child(10) > a";
...
export class HomePageParser {
  ...
  getCharacterLink(name: string, rankingPageHtml: string): string {
    try {
      ...
      const links: NhpHTMLElement[] = node.querySelectorAll(
        CHARACTER_LINKS_SELECTOR
      );
      const link = links.find(
        (linkNode: NhpHTMLElement) =>
          linkNode.innerText.toLowerCase() === name.toLowerCase()
      );
      ...
      if (error instanceof NotFoundError) {
        console.error(
          "캐릭터를 찾을수 없습니다, 홈페이지 티에스:${error.characterName}"
        );
      } else {
  ...
  ensureIsPublic(html: string, scope: string) {
    try {
      ...
      const privateDiv: NhpHTMLElement | null =
        node.querySelector("div.private2");
      if (privateDiv) throw new PrivateInformationError(scope);
    } catch (error) {
      if (error instanceof PrivateInformationError) {
        console.error(
          "캐릭터가 비공개, 홈페이지 티에스 ensureIsPublic : ${error.scope}"
        );
      } else {
        console.error(
          "캐릭터가 비공개, 홈페이지 티에스 ensureIsPublic : ",
          error
        );
      }
      return "";
  ...
  getEquipmentPageLink(characterLinkPageHtml: string): string {
    try {
      ...
      if (error instanceof NotValidSpecPageError) {
        console.error(
          "장비를 찾을수 없습니다, 홈페이지 티에스 getEquipmentPageLink"
        );
      } else {
        console.error(
          "장비를 찾을수 없습니다, 홈페이지 티에스 getEquipmentPageLink :",
  ...
  getPetPageLink(characterLinkPageHtml: string): string {
    try {
      ...
      if (error instanceof NotValidSpecPageError) {
        ...
        console.error(
          "펫을 찾을수 없습니다, 홈페이지 티에스 getPetPageLink:",
          error
        );
      }
      return "";
  ...
  getEquipmentLinks(equipmentPageHtml: string): EquipmentLinks {
    ...
    const baseLinks: NhpHTMLElement[] = node.querySelectorAll(
      BASE_EQUIPMENT_LINKS_SELECTOR
    );
    const cashLinks: NhpHTMLElement[] = node.querySelectorAll(
      CASH_EQUIPMENT_LINKS_SELECTOR
    );
    const symbolLinks: NhpHTMLElement[] = node.querySelectorAll(
      SYMBOL_EQUIPMENT_LINKS_SELECTOR
    );
