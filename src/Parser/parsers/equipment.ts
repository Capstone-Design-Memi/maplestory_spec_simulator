...
const ITEM_CATEGORY_SELECTOR =
  "div.item_ability > div:nth-child(3) > span > em";
const ITEM_LEVEL_SELECTOR =
  "div.item_ability > div:nth-child(1) > ul > li:nth-child(1) > em";
const ITEM_OPTIONS_SELECTOR = "div.stet_info > ul > li";
const ITEM_GRADE_SELECTOR =
  "div.item_title > div.item_memo > div.item_memo_sel";
...
type SymbolOption = Record<
  "level" | "experience" | "requiredExperience",
  number
>;
...
export class EquipmentParser {
  ...
  parseBase(equipmentHtml: string): Equipment {
    ...
    const { base, scroll, flame, potential, additional, soul, scissors } =
      this.parseOptions(node);
  ...
  private parseSymbolOptions(node: HTMLElement): SymbolOption {
    const [levelNode, experienceNode]: HTMLElement[] = node.querySelectorAll(
      ITEM_OPTIONS_SELECTOR
    );
    const level = levelNode.querySelector("div.point_td")?.text || "0";
    const experiences = experienceNode.querySelector("div.point_td")?.text;
    const [experience, requiredExperience] = experiences?.split("/") || [
      "0",
      "0",
    ];
    return {
  ...
  private parseLevel(node: HTMLElement): number {
    return Number.parseInt(
      node.querySelector(ITEM_LEVEL_SELECTOR)?.text.trim() ?? "0"
    );
  }

  private parseOptions(node: HTMLElement): EquipmentOption {
    const optionNodes: HTMLElement[] = node.querySelectorAll(
      ITEM_OPTIONS_SELECTOR
    );
  ...
  private parseName(
    node: HTMLElement,
    html: string
  ): { name: string; upgrade: number; star: number } {
    try {
