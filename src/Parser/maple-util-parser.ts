...
type EquipmentsResult = {
  success: Equipments;
  error: {
    [key in keyof Equipments]?: () => Promise<NonNullable<Equipments[key]>>;
  };
};
...
export class MapleUtilsParser {
  ...
  private async getCharacterInfo({
    ...
    const generalInformation =
      this.generalInformationParser.parse(characterSpecPage);

    const equipmentLink =
      this.homePageParser.getEquipmentPageLink(characterSpecPage);
  ...
  async getCharacter({
    ...
    const { spec, generalInformation, equipments, petEquipments } =
  ...
  private async getCharacterLink(name: string): Promise<string> {
    ...
    console.log(check);
    if (check == "reboot") {
      rankingSearch = await fetch(
        `${MAPLESTORY_RANKING_SEARCH}?c=${encodeURI(name)}&w=254`
      );
    } else {
      rankingSearch = await fetch(
        `${MAPLESTORY_RANKING_SEARCH}?c=${encodeURI(name)}`
      );
    }
  ...
  private async getSpecPage(characterLink: string): Promise<string> {
    try {
      ...
      if (characterSpecPage.status !== 200)
        throw new OpenPageError("캐릭터 정보");
      ...
      if (error instanceof OpenPageError) {
        ...
        console.error(
          "페이지 오류, 메이플 유틸 파서 티에스 getSpecPage : ",
          error
        );
      }
      return "";
  ...
  private async getEquipments(
    ...
    const equipmentLinks =
      this.homePageParser.getEquipmentLinks(equipmentPageData);
  ...
  private async getPetEquipments(
    petEquipmentLink: string
  ): Promise<CashEquipmentsResult> {
    ...
    const equipmentLinks: string[] =
      this.homePageParser.getPetEquipmentLinks(equipmentPageData);
    const equipmentHtml = await this.getAllHtmls(equipmentLinks);

    return {
      success: equipmentHtml.success
        .map((html) => this.equipmentParser.parseCash(html))
        .filter((e) => !!e),
  ...
  private async getAllHtmls(
    ...
    while (failedLinks.length > 0 && count < 5) {
      if (count) {
        console.log(
          `시도 횟수: ${count}, 남은 링크 수: ${failedLinks.length} ${count}초후 재시도`
        );
